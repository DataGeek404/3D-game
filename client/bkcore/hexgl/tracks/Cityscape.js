...
bkcore.hexgl.tracks.Cityscape = {
	...
	spawnMultiplayer: [
		{ x: -1134*2, y: 385, z: -443*2 },
		{ x: -1134*2 + 5, y: 385, z: -443*2 },
		{ x: -1134*2 - 5, y: 385, z: -443*2 },
		{ x: -1134*2 + 5, y: 385, z: -443*2 - 10 },
		{ x: -1134*2 - 5, y: 385, z: -443*2 - 10 }
	],
		...
		if(quality < 2) // LOW
		{
			this.lib.load({
				...
				analysers: {
					...
					'track.cityscape.rank'				: "textures/tracks/cityscape/ranking.png",
				},
				images: {
	...
	buildScenes: function(ctx, quality)
	{
		...
		shipControls.rankMap = this.lib.get("analysers", "track.cityscape.rank");
		ctx.components.shipControls = shipControls;
		ctx.tweakShipControls(shipControls);

		
		
		ctx.components.opponentControls = []
		ctx.components.opponentEffects = []

		socket.on('getOpponents', ({opponents}) => {
			opponents.forEach((opponent) => {
				createOpponent(opponent)
			})
		})

		socket.on('newOpponent', ({opponent}) => createOpponent(opponent))
		
		const createOpponent = (opponent) => {
			var opp = ctx.createMesh(scene, this.lib.get("geometries", "ship.feisar"), -1134*2, 10, -443*2, this.materials.ship);
			var boosterOpp = ctx.createMesh(opp, this.lib.get("geometries", "booster"), 0, 0.665, -3.8, this.materials.booster);
			boosterOpp.depthWrite = false;
			var boosterSpriteOpp = new THREE.Sprite({
				map: this.lib.get("textures", "booster.sprite"),
				blending: THREE.AdditiveBlending,
				useScreenCoordinates: false,
				color: 0xffffff,
			});
			boosterSpriteOpp.scale.set(0.02, 0.02, 0.02);
			boosterSpriteOpp.mergeWith3D = false;
			boosterOpp.add(boosterSpriteOpp);
			var boosterLightOpp = new THREE.PointLight(0x00a2ff, 4.0, 60);
			boosterLightOpp.position.set(0, 0.665, -4);

			var oppControls = new bkcore.hexgl.OpponentControls(ctx);
			oppControls.collisionMap = this.lib.get("analysers", "track.cityscape.collision");
			oppControls.collisionPixelRatio = 2048.0 / 6000.0;
			oppControls.collisionDetection = true;
			oppControls.heightMap = this.lib.get("analysers", "track.cityscape.height");
			oppControls.heightPixelRatio = 2048.0 / 6000.0;
			oppControls.heightBias = 4.0;
			oppControls.heightScale = 10.0;
			oppControls.socket_id = opponent.socket_id
			oppControls.positionIndex = opponent.rank
			oppControls.control(opp)
			ctx.components.opponentControls.push(oppControls);
			ctx.tweakShipControls(oppControls);

			if(quality > 0) {
					opp.add(boosterLightOpp);
			}

			var oppFxParams = {
				scene: scene,
				shipControls: oppControls,
				booster: boosterOpp,
				boosterSprite: boosterSpriteOpp,
				boosterLight: boosterLightOpp,
				useParticles: false
			};
			...
			ctx.components.opponentEffects.push(new bkcore.hexgl.OpponentEffects(oppFxParams))

			socket.on('opponentDisconnect', ({socket_id}) => {
				if (socket_id == oppControls.socket_id) {
					if (socket_id == oppControls.socket_id) {
						var index = null;
	
						ctx.components.opponentControls.forEach((opp, idx) => {
							if (opp.socket_id == socket_id) {
								index = idx
							}
						})
	
						if (index !== null) {
							ctx.components.opponentEffects.splice(index, 1)
							ctx.components.opponentControls.splice(index, 1)
						}
					}
					scene.remove(opp)
				}
		...
		ctx.manager.add("game", scene, camera, function(delta, renderer)
		{
			...
			if (window.hexGL.multiplayer == true) {
				this.objects.components.opponentControls.forEach((opponent) => {
					opponent.update(dt)
				})
				this.objects.components.opponentEffects.forEach((opponent) => {
					opponent.update(dt);
				})
			}
