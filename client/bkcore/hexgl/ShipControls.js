...
bkcore.hexgl.ShipControls = function(ctx)
{
	...
	socket.on('spawnPosition', ({position}) => {
	...
	function onKeyDown(event)
	{
		if (window.hexGL.multiplayer == true) {
			socket.emit('keydown', {keyCode: event.keyCode, position, velocity, rotation, quaternion});
		}
	...
	{
		if (window.hexGL.multiplayer == true) {
			socket.emit('keyup', {keyCode: event.keyCode, position, velocity, rotation, quaternion});
		}
...
bkcore.hexgl.ShipControls.prototype.update = function(dt)
{
	...
	quaternion.copy(this.dummy.quaternion)
