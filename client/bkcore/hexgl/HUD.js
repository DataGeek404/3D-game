...
bkcore.hexgl.HUD = function(opts)
{
	...
	this.leaderboardMarginRatio = 50
	...
	socket.on('updateLeaderboard', ({rankings}) => {
...
bkcore.hexgl.HUD.prototype.updateLeaderboard = function(rankings)
{
...
bkcore.hexgl.HUD.prototype.update = function(speed, speedRatio, shield, shieldRatio)
{
	...
	else if(this.step == 1)
	{
		...
		{
			this.ctx.font = (SCREEN_WIDTH/this.timeFontRatio)/1.5+"px "+this.font;
		    this.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
			this.ctx.textAlign = "left"; 
		    this.ctx.fillText("Leaderboard", SCREEN_WIDTH/this.leaderboardMarginRatio, SCREEN_WIDTH/this.timeMarginRatio);

			this.ctx.font = (this.width/this.timeFontRatio)/2 +"px "+this.font;
			this.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
			this.ctx.textAlign = "left"; 
			count = 1;
			this.rankings.forEach((ele) => {
				if (ele.name == window.hexGL.username)
					this.ctx.fillStyle = "rgba(153, 204, 255, 0.8)";
				else
					this.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";

				this.ctx.fillText(ele.name, SCREEN_WIDTH/this.leaderboardMarginRatio, (this.width * count)/(this.timeMarginRatio * 2) + this.width * 1.1/this.timeMarginRatio);
				this.ctx.fillText(ele.rank.toString(), this.width * 10/this.leaderboardMarginRatio, (this.width * count)/(this.timeMarginRatio * 2) + this.width * 1.1/this.timeMarginRatio);

				count++;
			})

			if (this.rankings.length < 4 && this.visible && window.hexGL.gameplay.active == false) {
				this.ctx.textAlign = "center"; 
				this.ctx.font = (this.width/this.timeFontRatio)/1.5 +"px "+this.font;
				this.ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
				this.ctx.fillText(`Waiting for players... (${this.rankings.length}/${this.multiplayerCount}) joined`, SCREEN_HW, SCREEN_HH); 
			} 
			this.ctx.textAlign = "center"; 
		}
